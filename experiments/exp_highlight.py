...
# assumes that this python file is run from ./experiments dir.
sys.path.append('../')
...
def run_exp_pipeline():
    ...
    '''
    # directory for saving the outputs of the experiment
    ...
    if opt.sampler_type == "PLMSSampler":
        sampler = PLMSSampler(
            ...
            sd_weights_path=opt.sd_weights_path,
            verbose=opt.verbose,
    ...
    for batch_idx, batch in tqdm(enumerate(dataloader)):
        ...
        #print('scribble_masks : ', len(scribble_masks))
        
        # preprocessed arguments as A
        A = preprocess_arguments(
                models = models,
                arguments = {
                    'prompt' : prompt[0],
        ...
        # preprocess twice, which is inefficient.
        ...
        sample = x0[0].cpu().numpy().transpose(1, 2, 0) * 255
        ...
        with torch.no_grad():
            ...
            x0_sample = (x0.numpy().transpose(1,2,0)*255).astype(np.uint8)
            ...
            try:
                iou_dict = evaluate_propagation_mIOU(original_scribbles=A['scribble_list'][0],
